...
export default function RutTienPage() {
  ...
  const availableBalance = 45750000;
  const pendingBalance = 12500000;

  const quickSet = (v) => setAmount(v);
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!amount || !account) return;
    const fee = Math.min(Math.round(amount * 0.005), 75000);
    const newItem = {
      id: Date.now(),
      method: account.includes("MoMo") ? "MoMo" : "Vietcombank",
      masked: account.match(/\*+/)?.[0] || "****0000",
      status: "Đang xử lý",
          ...
          <section className="wd-request gv-card">
            ...
            <form onSubmit={handleSubmit} className="wd-form">
              ...
              <div className="wd-all">
                <button type="button" onClick={() => quickSet(availableBalance)}>
              ...
              <div className="wd-label-row">
                ...
                <button type="button" className="wd-add-account">+ Thêm tài khoản</button>
              </div>
              <select className="wd-select" value={account} onChange={(e) => setAccount(e.target.value)}>
                ...
                <option value="VCB ****6789">Vietcombank ****6789</option>
                <option value="MoMo ****4321">MoMo ****4321</option>
              </select>
          ...
          <section className="wd-history gv-card">
            <div className="wd-history-head">
              ...
              <div className="wd-tools">
                <button className="btn btn-light" onClick={() => setFilterOpen((v)=>!v)}>{filterOpen ? "Đóng" : "Lọc"}</button>
                <button className="btn btn-light" onClick={exportCSV}>Xuất Excel</button>
              </div>
            </div>

            {filterOpen && (
              <div className="wd-filter">
                <div className="wd-filter-grid">
                  <div>
                    <label className="wd-label">Phương thức</label>
                    <select className="wd-select" value={filters.method} onChange={(e)=>setFilters(f=>({...f, method:e.target.value}))}>
                  ...
                  <div>
                    <label className="wd-label">Trạng thái</label>
                    <select className="wd-select" value={filters.status} onChange={(e)=>setFilters(f=>({...f, status:e.target.value}))}>
                      <option value="">Tất cả</option>
                  ...
                  <div>
                    <label className="wd-label">Từ ngày</label>
                    <input className="wd-input" type="date" value={filters.from} onChange={(e)=>setFilters(f=>({...f, from:e.target.value}))} />
                  </div>
                  <div>
                    <label className="wd-label">Đến ngày</label>
                    <input className="wd-input" type="date" value={filters.to} onChange={(e)=>setFilters(f=>({...f, to:e.target.value}))} />
                  </div>
                  <div>
                    <label className="wd-label">Số tiền tối thiểu</label>
                    <input className="wd-input" type="number" value={filters.min} onChange={(e)=>setFilters(f=>({...f, min:e.target.value}))} placeholder="vd: 1000000" />
                  </div>
                  <div>
                    <label className="wd-label">Số tiền tối đa</label>
                    <input className="wd-input" type="number" value={filters.max} onChange={(e)=>setFilters(f=>({...f, max:e.target.value}))} placeholder="vd: 20000000" />
                  </div>
                  <div className="wd-filter-q">
                    <label className="wd-label">Tìm (Mã GD/Tài khoản)</label>
                    <input className="wd-input" type="text" value={filters.q} onChange={(e)=>setFilters(f=>({...f, q:e.target.value}))} placeholder="Nhập mã giao dịch hoặc ****xxxx" />
                  </div>
                </div>
                <div className="wd-filter-actions">
                  <button className="btn btn-light" onClick={resetFilters}>Xóa bộ lọc</button>
                  <button className="btn btn-primary" onClick={()=>setFilterOpen(false)}>Áp dụng</button>
                </div>
              </div>
            )}
