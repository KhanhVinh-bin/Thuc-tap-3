  ...
  useEffect(() => {
    ...
    const fetchInstructor = async () => {
      ...
      try {
        ...
        setInstructor({
          id: formatInstructorId(data.ID ?? data.id),
          name: data.FullName ?? data.fullName ?? data.name ?? '',
          email: data.Email ?? data.email ?? '',
          phone: data.PhoneNumber ?? data.phoneNumber ?? data.phone ?? '',
          address: data.Address ?? data.address ?? '',
          specialty: data.Expertise ?? data.expertise ?? '',
          education: data.Education ?? data.education ?? '—',
          joinDate: data.CreatedAt ? formatDate(data.CreatedAt) : (data.createdAt ? formatDate(data.createdAt) : ''),
          experience: (data.ExperienceYears ?? data.experienceYears) != null ? `${data.ExperienceYears ?? data.experienceYears} năm` : '',
          status: mapStatusToVi(data.Status ?? data.status),
          statusRaw: (data.Status ?? data.status) || 'active',
          courses: data.Courses ?? data.courses ?? [],
          rating: data.RatingAverage ?? data.ratingAverage ?? 0,
          avatar: data.AvatarUrl ?? data.avatarUrl ?? '/placeholder-user.jpg',
                      ...
                      <table className="data-table courses-table">
                        ...
                        <tbody>
                          {instructor.courses.map((course, index) => (
                            <tr key={index}>
                              <td>{course.Title ?? course.title ?? course.name}</td>
                              <td>{formatDate(course.CreatedAt ?? course.createdAt)}</td>
                              <td>{course.TotalStudents ?? course.totalStudents ?? 0}</td>
                              <td>
                                <span className={`status-badge status-${(course.Status ?? course.status ?? 'active').toLowerCase()}`}>
                                  {mapCourseStatusToVi(course.Status ?? course.status)}
                                </span>
                              </td>
                              <td>
