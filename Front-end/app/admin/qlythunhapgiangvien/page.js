...
export default function InstructorIncomeManagement() {
  ...
  const fetchPayouts = async () => {
    ...
    try {
      ...
      const mapped = Array.isArray(data)
          ? data.map((p) => {
              ...
              return {
                  ...
                  revenue: formatCurrency(p.CourseRevenue ?? p.courseRevenue),
                  commission: `${Number((p.CommissionPercent ?? p.commissionPercent) || 0)}%`,
                  fee: formatCurrency(p.AmountHeld ?? p.amountHeld),
                  payout: formatCurrency(p.PayoutAmount ?? p.payoutAmount),
      ...
      let filtered = term
          ? mapped.filter((i) =>
              [i.instructor, i.email, i.course].some((f) => (f || '').toLowerCase().includes(term))
            )
          : mapped;
          ...
          <tbody>
            {instructorIncomes.map((income) => (
              <tr key={income.id}>
                ...
                <td><span className="course-name">{income.course}</span></td>
                <td><span className="revenue">{income.revenue}</span></td>
                <td><span className="commission">{income.commission}</span></td>
