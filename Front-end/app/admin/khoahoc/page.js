...
import { Plus, RefreshCw } from 'lucide-react'; // thêm nếu dùng icon
...
// Handlers nằm trong component để dùng router/fetchCourses
const handleAddCourse = () => {
router.push('/admin/khoahoc/taomoi'); // đổi route nếu khác
};
// Thống kê nhanh dựa trên danh sách courses
const totalCount = courses.length;
// Hoạt động: gồm 'active' và 'published'
const activeCount = courses.filter((c) => {
  const s = (c.status || '').toLowerCase();
  return s === 'active' || s === 'published';
}).length;
// Chờ duyệt
const pendingCount = courses.filter((c) => (c.status || '').toLowerCase() === 'pending').length;
// Bản nháp
const draftCount = courses.filter((c) => (c.status || '').toLowerCase() === 'draft').length;
const handleRefresh = () => {
fetchCourses();
};
  const fetchCourses = async () => {
    ...
    try {
      // Sử dụng đúng endpoint theo backend: /admin/courses và áp dụng bộ lọc
      const params = new URLSearchParams();
      if (search && search.trim()) {
        params.set('title', search.trim());
      }
      // Map trạng thái UI sang giá trị backend
      const statusMap = {
        'Hoạt động': 'active',
        'Chờ duyệt': 'pending',
        'Bản nháp': 'draft',
      ...
      const url = `${API_BASE}/admin/courses${params.toString() ? `?${params.toString()}` : ''}`;
      ...
      // Chỉ parse JSON khi response ok và có Content-Type là JSON
      if (!response.ok) {
      ...
      // Backend trả về dạng { items: [...] }
      const items = Array.isArray(data?.items) ? data.items : Array.isArray(data) ? data : [];
      const normalized = items.map((c) => ({
        id: c.ID ?? c.id ?? null,
        tenKhoaHoc: c.TenKhoaHoc ?? c.tenKhoaHoc ?? '',
        giangVien: c.GiangVien ?? c.giangVien ?? null,
        thoiLuong: c.ThoiLuong ?? c.thoiLuong ?? null,
        soHocVien: c.SoHocVien ?? c.soHocVien ?? 0,
        ngayBatDau: c.NgayBatDau ?? c.ngayBatDau ?? null,
        status: c.Status ?? c.TrangThai ?? c.status ?? null,
      }));
  ...
  const handleSeedData = async () => {
    try {
      const url = `${API_BASE}/admin/courses/seed`;
      const response = await fetch(url, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleDeleteCourse = async (courseId, courseName) => {
    ...
    try {
      const url = `${API_BASE}/admin/courses/${courseId}`;
      const response = await fetch(url, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
    ...
    <>
      <div className="page-header">
        <div className="header-left">
          ...
          <p>Theo dõi, lọc và quản lý các khóa học trong hệ thống</p>
          <div className="stat-chips">
            <span className="stat-chip total">Tổng: {totalCount}</span>
            <span className="stat-chip active">Hoạt động: {activeCount}</span>
            <span className="stat-chip pending">Chờ duyệt: {pendingCount}</span>
            <span className="stat-chip pending">Nháp: {draftCount}</span>
          </div>
        </div>
        <div className="header-right">
          <button className="btn-add-course" onClick={handleAddCourse}>
            <Plus size={16} className="btn-icon" /> Thêm khóa học
          </button>
      ...
      <div className="controls-section">
            ...
            <div className="filter-controls">
              ...
              <button className="btn btn-primary" onClick={fetchCourses}>Tìm kiếm</button>
            </div>
          </div>
          ...
          <div className="course-count">
            ...
            {courses.length === 0 && (
              <button className="btn btn-primary" onClick={handleSeedData}>Tạo dữ liệu mẫu</button>
            )}
          </div>
                    ...
                    <tr key={course.id}>
                      ...
                      <td>
                        <span className={`status-badge ${(() => {
                          ...
                          if (s === 'active' || s === 'published') return 'status-active';
                          if (s === 'pending' || s === 'draft') return 'status-pending';
                          return 'status-inactive';
                        })()}`}>
                          {(() => {
                            const s = (course.status || '').toLowerCase();
                            if (s === 'active' || s === 'published') return 'Đang hoạt động';
