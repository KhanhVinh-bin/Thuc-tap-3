...
import "../tongquan/page.css"; // tái sử dụng layout chung (topbar, sidebar, grid)
import "./page.css"; // style riêng cho trang Hỗ trợ
import Footer from "@/components/footer";
import { useState } from "react";
import { useAuth } from "@/lib/auth-context";
import { sendSupportMessage } from "../lib/instructorApi";

export default function GiangVienHoTroPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token) {
      setErrorMsg("Chưa đăng nhập hoặc thiếu token. Vui lòng đăng nhập lại.");
      return;
    }
    try {
      setSubmitting(true);
      const res = await sendSupportMessage(token, { fullName, email, subject, content });
      setSuccessMsg(res?.message || "Gửi yêu cầu hỗ trợ thành công.");
      // Clear form
      setFullName("");
      setEmail("");
      setSubject("");
    ...
    <div className={`gv-dashboard-root ${sidebarCollapsed ? 'collapsed' : ''}`}>
        ...
        <div className="gv-topbar-left">
          ...
          <div className="gv-breadcrumb" aria-label="Breadcrumb"> 
            ...
            > 
             ...
             <span className="gv-bc-label">Hỗ trợ</span> 
          </div>
        </div>
      ...
      <div className="gv-dashboard">
        ...
        <aside className="gv-sidebar">
          <nav className="gv-nav">
            <ul>
              ...
              <li>
                <Link href="/giangvien/hotro" className="active">
                  <span aria-hidden="true" style={{ display: "inline-flex", alignItems: "center", gap: "6px" }}>
        ...
        <main className="gv-main">
          <h1 className="hs-title">Trung tâm hỗ trợ</h1>

          {/* FAQ */}
          <section className="hs-card">
            <div className="hs-section-title">Câu hỏi thường gặp</div>
            <div className="hs-accordion">
              {[
                "Làm thế nào để tạo khóa học mới?",
                "Làm thế nào để rút tiền?",
                "Tôi có thể thêm bao nhiêu bài học vào một khóa học?",
                "Làm thế nào để tương tác tốt với học viên?",
                "Tỷ lệ chia sẻ doanh thu là bao nhiêu?",
                "Làm thế nào để tăng lượt đăng ký khóa học?",
              ].map((q, idx) => (
                <details key={idx} className="hs-item">
                  <summary>
                    <span className="hs-question">{q}</span>
                    <span className="hs-caret" aria-hidden="true">
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24">
                        <path d="M7 10l5 5 5-5" fill="none" stroke="#6b7280" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </span>
                  </summary>
                  <div className="hs-answer">
                    Nội dung hướng dẫn ngắn sẽ hiển thị tại đây. Liên hệ chúng tôi nếu cần thêm hỗ trợ.
                  </div>
                </details>
              ))}
            </div>
          ...
          <section className="hs-card">
            <form className="hs-form" onSubmit={handleSubmit}>
              ...
              <button type="submit" className="hs-submit" disabled={submitting}>
                <span className="hs-plane" aria-hidden="true">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"><path d="M2 21l21-9L2 3v7l15 2-15 2v7z" fill="#ffffff"/></svg>
                </span>
