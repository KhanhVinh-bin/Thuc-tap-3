...
export default function GiangVienDoanhThuPage() {
  ...
  const [courseSold, setCourseSold] = useState(0)
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setError(null)
        if (!token) {
          throw new Error("Chưa đăng nhập hoặc thiếu token")
        }

        const [summary, month, chart, balance] = await Promise.all([
          getRevenueSummary(token),
          getRevenueCurrentMonth(token),
          getRevenueChart6Months(token),
          getInstructorBalance(token),
        ])

        setTotalRevenue(summary?.totalRevenue || 0)
        setPercentChange(summary?.percentChange || 0)
        setCurrentMonthRevenue(month?.totalRevenue || 0)
        setCourseSold(month?.courseSold || 0)
        setChartMonths(Array.isArray(chart) ? chart : [])
        setAvailableBalance(balance?.AvailableBalance ?? balance?.availableBalance ?? 0)
      } catch (e) {
        console.error("Load doanh thu error:", e)
        setError(e?.message || "Không thể tải dữ liệu doanh thu")
      } finally {
        setLoading(false)
      }
    }
  ...
  const max = Math.max(...chartMonths.map((m) => Number(m.revenue || 0)), 0)

  const chartData = {
    labels: chartMonths.map((m) => String(m.label || "")),
    datasets: [
      {
        label: "Doanh thu",
        data: chartMonths.map((m) => Number(m.revenue || 0)),
  ...
  const chartOptions = {
    ...
    plugins: {
      tooltip: {
        callbacks: {
          label: (ctx) => `${Number(ctx.parsed.y || 0).toLocaleString('vi-VN')} VND`,
        },
      },
    ...
    scales: {
      ...
      y: {
        beginAtZero: true,
        min: 0,
        max: 50000000, // 50 triệu VND
        ticks: {
          stepSize: 10000000, // mỗi vạch 10 triệu
          callback: (value) => `${(Number(value) / 1000000).toLocaleString('vi-VN')} triệu vnđ`,
        },
      },
    },
  }
    ...
    <div className={`gv-dashboard-root ${sidebarCollapsed ? "collapsed" : ""}`}>
        ...
        <div className="gv-topbar-left">
          ...
          <div className="gv-breadcrumb" aria-label="Breadcrumb"> 
            ...
            > 
             ...
             <span className="gv-bc-label">Doanh thu</span> 
          </div>
        </div>
      ...
      <div className="gv-dashboard">
                ...
                <Link href="/giangvien/hotro">
                  <span aria-hidden="true" style={{ display: "inline-flex", alignItems: "center", gap: "6px" }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" aria-hidden="true">
                      <path d="M8.05 9.6
                               c.336 0 .504-.24.554-.627
                               .04-.534.198-.815.847-1.26
                               .673-.475 1.049-1.09 1.049-1.986
                               0-1.325-.92-2.227-2.262-2.227
                               -1.02 0-1.792.492-2.1 1.29
                               A1.71 1.71 0 0 0 6 5.48
                               c0 .393.203.64.545.64
                               .272 0 .455-.147.564-.51
                               .158-.592.525-.915 1.074-.915
                               .61 0 1.03.446 1.03 1.084
                               0 .563-.208.885-.822 1.325
                               -.619.433-.926.914-.926 1.64v.111
                               c0 .428.208.745.585.745z"/>
                      {/* Viền huy hiệu (hình hoa / sao mềm) */}
                      <path d="m10.273 2.513-.921-.944.715-.698.622.637.89-.011
                               a2.89 2.89 0 0 1 2.924 2.924l-.01.89.636.622
                               a2.89 2.89 0 0 1 0 4.134l-.637.622.011.89
                               a2.89 2.89 0 0 1-2.924 2.924l-.89-.01-.622.636
                               a2.89 2.89 0 0 1-4.134 0l-.622-.637-.89.011
                               a2.89 2.89 0 0 1-2.924-2.924l.01-.89-.636-.622
                               a2.89 2.89 0 0 1 0-4.134l.637-.622-.011-.89
                               a2.89 2.89 0 0 1 2.924-2.924l.89.01.622-.636
                               a2.89 2.89 0 0 1 4.134 0l-.715.698
                               a1.89 1.89 0 0 0-2.704 0l-.92.944-1.32-.016
                               a1.89 1.89 0 0 0-1.911 1.912l.016 1.318-.944.921
                               a1.89 1.89 0 0 0 0 2.704l.944.92-.016 1.32
                               a1.89 1.89 0 0 0 1.912 1.911l1.318-.016.921.944
                               a1.89 1.89 0 0 0 2.704 0l.92-.944 1.32.016
                               a1.89 1.89 0 0 0 1.911-1.912l-.016-1.318.944-.921
                               a1.89 1.89 0 0 0 0-2.704l-.944-.92.016-1.32
                               a1.89 1.89 0 0 0-1.912-1.911l-1.318.016z"/>
                      {/* Dấu chấm dưới dấu hỏi */}
        ...
        <main className="gvr-main">
          {/* Summary cards */}
          <section className="gvr-top-cards">
            <div className="gvr-card green">
              <div className="gvr-card-head">
                <div className="gvr-card-title">Tổng doanh thu</div>
                <div className="gvr-card-icon" aria-hidden="true">
                  <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24"><path d="M12 3C7.03 3 3 7.03 3 12s4.03 9 9 9 9-4.03 9-9-4.03-9-9-9zm1 15h-2v-2H9v-2h2V8h2v4h2v2h-2v2z" fill="#ffffff"/></svg>
                </div>
              </div>
              <div className="gvr-card-value">{toVND(totalRevenue)}</div>
              <div className="gvr-card-sub">{percentChange >= 0 ? "↑" : "↓"} {Math.abs(percentChange).toFixed(0)}% So với tháng trước</div>
            </div>

            <div className="gvr-card blue">
              <div className="gvr-card-head">
                <div className="gvr-card-title">Số dư khả dụng</div>
                <div className="gvr-card-icon" aria-hidden="true">
                  <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4 13h-3v3h-2v-3H8v-2h3V10h2v3h3v2z" fill="#ffffff"/></svg>
                </div>
              </div>
              <div className="gvr-card-value">{toVND(availableBalance)}</div>
                          
                           <Link href="/giangvien/doanhthu/ruttien" className="gvr-btn">Rút tiền</Link>
            </div>

            <div className="gvr-card purple">
              <div className="gvr-card-head">
                <div className="gvr-card-title">Doanh thu tháng này</div>
                <div className="gvr-card-icon" aria-hidden="true">
                  <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24"><path d="M7 10l5 5 5-9" stroke="#ffffff" strokeWidth="2" fill="none"/></svg>
                </div>
              </div>
              <div className="gvr-card-value">{toVND(currentMonthRevenue)}</div>
              <div className="gvr-card-sub">{courseSold} khóa học đã được bán</div>
            </div>
          </section>

          {/* Chart */}
          <section className="gvr-chart">
            <div className="gvr-chart-title">Biểu đồ doanh thu (6 tháng)</div>
            <div className="gvr-chart-area" style={{ height: 300 }}>
