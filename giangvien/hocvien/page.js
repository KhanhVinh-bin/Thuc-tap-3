...
import { getInstructorStudents, searchInstructorStudents } from "../lib/instructorApi"

export default function GiangVienHocVienPage() {
  ...
  const [filterKey, setFilterKey] = useState("FullName")
  const [filterInput, setFilterInput] = useState("")
  const filterOptions = [
    { value: "FullName", label: "Tên học viên" },
    { value: "Email", label: "Email" },
    { value: "Title", label: "Tên khóa học" },
    { value: "Status", label: "Trạng thái" },
    { value: "EnrollDate", label: "Ngày đăng ký" },
  ]
  ...
  useEffect(() => {
    ...
    async function fetchStudents() {
      ...
      setError("")
      try {
        const arr = await getInstructorStudents(token)
        if (!cancelled) {
          setStudents(arr)
          setCurrentPage(1)
        }
        ...
        console.error(err)
        if (!cancelled) setError(err?.message || "Có lỗi xảy ra khi tải học viên")
      } finally {
  ...
  // Chuẩn hóa chuỗi ngày nhập để gửi lên API
  ...
  // Định dạng hiển thị Truy cập cuối, có fallback khi null
  function formatLastActive(v) {
    if (!v) return "Chưa truy cập"
    // Nếu là chuỗi thời gian, có thể để nguyên vì backend đã định dạng.
    // Tương lai: chuẩn hóa theo dd/MM/yyyy HH:mm nếu cần.
    return v
  }

  // Kiểm tra định dạng ngày nhập: năm (yyyy), MM/yyyy, dd/MM/yyyy, dd-MM-yyyy, yyyy-MM-dd
  function isValidEnrollDateInput(v) {
    if (!v) return false
    const s = String(v).trim()
    ...
    if (/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(s) || /^\d{1,2}-\d{1,2}-\d{4}$/.test(s)) {
  ...
  async function applySearch() {
    ...
    try {
      ...
      if (!hasAnyFilter) {
        ...
        const { Completion, ...rest } = filters
        const payload = { ...rest }
        // Kiểm tra định dạng ngày và gửi chuỗi thô
        if (payload.EnrollDate) {
          if (!isValidEnrollDateInput(payload.EnrollDate)) {
            setError("Định dạng ngày không hợp lệ. Vui lòng nhập: 2025, 11/2025 hoặc 01/11/2025.")
            setLoading(false)
            return
          }
          payload.EnrollDate = String(payload.EnrollDate).trim()
        }
        // Prefer dropdown completion over free-text status
        const statusToSend = (Completion && Completion.trim() !== "") ? Completion : payload.Status
        if (statusToSend && statusToSend.trim() !== "") payload.Status = statusToSend
        else delete payload.Status
        const arr = await searchInstructorStudents(token, payload)
        setStudents(arr)
        setCurrentPage(1)
      }
    } catch (err) {
  ...
  function applySimpleFilter() {
    // Đưa giá trị từ thanh chọn + ô nhập vào filters rồi gọi tìm kiếm
    setFilters(prev => {
      const cleared = { FullName: "", Email: "", Title: "", Status: "", EnrollDate: "", Completion: "" }
      cleared[filterKey] = filterInput
      return cleared
    })
    // Gọi tìm kiếm sau tick để đảm bảo state đã cập nhật
    setTimeout(() => applySearch(), 0)
  }

  function resetFilters() {
    setFilters({ FullName: "", Email: "", Title: "", Status: "", EnrollDate: "", Completion: "" })
    setFilterKey("FullName")
    setFilterInput("")
    applySearch()
  }

  // ---------- Pagination helpers ----------
  const totalPages = Math.max(1, Math.ceil((students?.length || 0) / PAGE_SIZE))
  ...
  const renderPages = () => {
    ...
    if (totalPages <= windowSize + 2) {
      for (let i = 1; i <= totalPages; i++) {
        items.push(
          <span key={i} className={`gvc-page ${i === safePage ? 'active' : ''}`} onClick={() => goToPage(i)}>{i}</span>
        )
      }
    } else {
      if (safePage <= 3) {
        ...
        items.push(<span key="ellipsis-right" className="gvc-page arrow">…</span>)
        items.push(
          <span key={totalPages} className={`gvc-page ${totalPages === safePage ? 'active' : ''}`} onClick={() => goToPage(totalPages)}>{totalPages}</span>
        )
      } else if (safePage >= totalPages - 2) {
        items.push(
          <span key={1} className={`gvc-page ${1 === safePage ? 'active' : ''}`} onClick={() => goToPage(1)}>1</span>
        )
        items.push(<span key="ellipsis-left" className="gvc-page arrow">…</span>)
    ...
    <div className={`gv-dashboard-root ${sidebarCollapsed ? "collapsed" : ""}`}>
      ...
      <div className="gv-dashboard">
        ...
        <main className="gv-content">
          <div className="gvs-card">
            <div className="gvs-card-header" style={{alignItems:"flex-start"}}>
              <div>
                <div className="gvs-title">Học viên</div>
                <div style={{marginTop:"14px",width:"100%"}}>
                  <div className="gvs-filter-bar">
                    <div className="gvs-select-wrap">
                      <Select value={filterKey} onValueChange={(v) => { setFilterKey(v); setFilterInput(""); }}>
                        <SelectTrigger className="gvs-select-trigger" aria-label="Chọn tiêu chí lọc">
                          <SelectValue placeholder="Chọn tiêu chí" />
                        </SelectTrigger>
                        <SelectContent className="gvs-select-content">
                    ...
                    {filterKey === "Status" ? (
                      <Select value={filterInput} onValueChange={v => setFilterInput(v)}>
                        <SelectTrigger className="gvs-input-inline gvs-select-trigger" aria-label="Chọn trạng thái">
                      ...
                      <input
                        className="gvs-input-inline"
                        type="text"
                        placeholder={
                          filterKey === "FullName" ? "Tên học viên" :
                          filterKey === "Email" ? "Email" :
                          filterKey === "Title" ? "Tên khóa học" :
                          filterKey === "EnrollDate" ? "Nhập ngày/tháng/năm hoặc năm (VD: 2025, 11/2025, 01/11/2025)" :
                          "Nhập giá trị"
                        }
                        value={filterInput}
                        onChange={e => setFilterInput(e.target.value)}
                        onKeyDown={(e) => { if (e.key === 'Enter') applySimpleFilter(); }}
                      />
                    )}
                    ...
                    <div className="gvs-actions">
                      <button
                        ...
                        className="gvs-btn gvs-btn--primary"
                      >
                        Tìm kiếm
                      </button>
                      <button
                        type="button"
                        onClick={() => { setFilterInput(""); setFilterKey("FullName"); resetFilters(); }}
                        className="gvs-btn gvs-btn--outline"
                      >
                        Xóa lọc
                      </button>
                    </div>
            ...
            {/* Rows */}
            <div className="gvs-table-body">
              {loading && (
                <div className="gvs-loading-overlay"><span>Đang tải dữ liệu…</span></div>
              )}
              {error && (
                <div className="gvs-row">
                  <div className="gvs-student" style={{color:'#b91c1c'}}>Lỗi: {error}</div>
                </div>
              )}
              {!loading && !error && students.length === 0 && (
                <div className="gvs-row">
                  <div className="gvs-student">Không có học viên nào</div>
                </div>
              )}
              {!error && pageStudents.map((s, i) => (
                <div key={i} className="gvs-row">
                  <div className="gvs-student">
                    <span className="gvs-avatar" aria-hidden="true">
                      <img src="/placeholder-user.jpg" alt="avatar" />
                    </span>
                    <div className="gvs-student-info">
                      <div className="gvs-student-name">{s.fullName || s.name}</div>
                      <div className="gvs-student-email">{s.email}</div>
                    </div>
                  </div>
                  <div className="gvs-course">
                    <div className="gvs-course-title">{s.courseTitle || s.course}</div>
                    <div className="gvs-course-date">{s.enrollmentDate ? `Đăng ký: ${s.enrollmentDate}` : ''}</div>
                  </div>
                  <div className="gvs-progress">
                    <div className="gvs-progress-top">
                      <span className="gvs-progress-percent">{s.progressPercent ?? 0}%</span>
                      <span className="gvs-progress-text">{s.progressText || ''}</span>
                    </div>
                    <div className="gvs-progress-bar">
                      <div className="gvs-progress-fill" style={{ width: `${s.progressPercent ?? 0}%` }} />
                    </div>
                  </div>
                  <div className="gvs-access">{formatLastActive(s.lastActive || s.lastAccess)}</div>
                  <div className="gvs-status">
                    <span className={`gvs-pill ${(s.enrollmentStatus?.type) || (s.status?.type) || 'inactive'}`}>{(s.enrollmentStatus?.label) || (s.status?.label) || 'Không hoạt động'}</span>
                  </div>
                </div>
              ))}
            </div>

            {/* Pagination - single cluster centered via absolute wrapper */}
            <div className="gvc-pagination">
              <div className="gvc-pagination-inner">
                <span
                  className="gvc-page arrow"
                  onClick={() => goToPage(1)}
