...
import "./page.css"
// Xóa biểu đồ giả, chỉ dùng biểu đồ thật từ API
// import { ChartAreaIcon, ChartSparklineIcon } from "./chart-icons.js"
import {
  getDashboardMonthlyRevenue,
  getDashboardCompletionRate,
  getDashboardTotalCourses,
  getDashboardTotalStudents,
  getDashboardRecentCourses,
  getDashboardRecentActivities,
} from "../lib/instructorApi"
import StudentGrowthAreaChart from "./StudentGrowthAreaChart"
import RevenueAreaChart from "./RevenueAreaChart"
import CompletionAreaChart from "./CompletionAreaChart"
import CourseGrowthAreaChart from "./CourseGrowthAreaChart"
...
export default function GiangVienTongQuanPage() {
  ...
  const [coursesPercent, setCoursesPercent] = useState(0)
  const [totalStudents, setTotalStudents] = useState(0)
  const [studentsPercent, setStudentsPercent] = useState(0)
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        setLoading(true)
        const [rev, comp, courses, students, rcourses, racts] = await Promise.all([
          getDashboardMonthlyRevenue(token),
          getDashboardCompletionRate(token),
          getDashboardTotalCourses(token),
          getDashboardTotalStudents(token),
          getDashboardRecentCourses(token),
          getDashboardRecentActivities(token),
        ])

        // Revenue
        setMonthlyRevenue(Number(rev?.currentMonthRevenue || 0))
        setRevenueDelta(Number(rev?.revenueDifference || 0))

        // Completion
        setCompletionRate(Number(comp?.currentRate || 0))
        setCompletionDelta(Number(comp?.difference || 0))

        // Courses
        setTotalCourses(Number(courses?.totalCourses || 0))
        setCoursesPercent(Number(courses?.percentChange || 0))

        // Students
        setTotalStudents(Number(students?.totalStudents || 0))
        setStudentsPercent(Number(students?.percentChange || 0))

        // Lists
        setRecentCourses(Array.isArray(rcourses) ? rcourses : [])
        setRecentActivities(Array.isArray(racts) ? racts : [])
      } catch (err) {
  ...
  // Map status to Vietnamese label + class
  const mapStatus = (status) => {
    const s = String(status || '').toLowerCase()
    if (s.includes('publish')) return { label: 'Đã xuất bản', cls: 'published' }
    if (s.includes('draft')) return { label: 'Bản nháp', cls: 'draft' }
    if (s.includes('pending')) return { label: 'Chờ duyệt', cls: 'pending' }
    if (s.includes('reject')) return { label: 'Bị trả về', cls: 'rejected' }
    return { label: status || 'Trạng thái', cls: 'draft' }
  }

  // Map activity type/message to dot color per mockup
  const dotClassForActivity = (type, message) => {
    const t = String(type || '').toLowerCase()
    if (t === 'enrollment') return 'green'
    if (t === 'review') return 'purple'
    if (t === 'course') {
      const m = String(message || '').toLowerCase()
      if (m.includes('chờ') || m.includes('pending')) return 'violet'
      if (m.includes('trả về') || m.includes('rejected') || m.includes('chỉnh sửa')) return 'yellow'
      return 'lime'
    }
    return 'green'
  }
        ...
        <main className="gv-main">
          ...
          <section className="gv-top-grid">
            <div className="gv-card gv-revenue">
              ...
              <div className="gv-card-value">{formatCurrency(monthlyRevenue)}</div>
              <div className={`gv-change ${revenueDelta >= 0 ? 'up' : 'down'}`}>
                <span className="gv-arrow">{revenueDelta >= 0 ? '↑' : '↓'}</span>
                <span>
                  {formatCurrency(Math.abs(revenueDelta))} <span className="gv-note">So với tháng trước</span>
                </span>
              </div>
              <div className="gv-card-chart">
                <RevenueAreaChart height={140} />
              </div>
            </div>
            ...
            <div className="gv-card gv-complete">
              ...
              <div className="gv-card-value">{Number(completionRate || 0).toFixed(0)}%</div>
              <div className={`gv-change ${completionDelta >= 0 ? 'up' : 'down'}`}>
                <span className="gv-arrow">{completionDelta >= 0 ? '↑' : '↓'}</span>
                <span>
                  {Math.abs(Number(completionDelta || 0)).toFixed(2)}% <span className="gv-note">So với kì trước</span>
                </span>
              </div>
              <div className="gv-card-chart">
                <CompletionAreaChart height={140} />
              </div>
            </div>
          </section>
          ...
          <section className="gv-mid-grid">
            <div className="gv-card">
              ...
              <div className="gv-card-value">{totalCourses}</div>
              <div className={`gv-card-desc ${coursesPercent >= 0 ? 'up' : 'down'}`}>{coursesPercent >= 0 ? '↑' : '↓'} {Math.abs(Number(coursesPercent || 0)).toFixed(2)}% So với tháng trước</div>
              <div className="gv-card-chart">
                <CourseGrowthAreaChart months={6} height={120} />
              </div>
            </div>
            <div className="gv-card">
              <div className="gv-card-title">Tổng học viên</div>
              <div className="gv-card-value">{new Intl.NumberFormat('vi-VN').format(totalStudents)}</div>
              <div className={`gv-card-desc ${studentsPercent >= 0 ? 'up' : 'down'}`}>{studentsPercent >= 0 ? '↑' : '↓'} {Math.abs(Number(studentsPercent || 0)).toFixed(2)}% So với tháng trước</div>
              <div className="gv-card-chart">
                <StudentGrowthAreaChart months={6} height={120} />
              </div>
            </div>
          </section>
            ...
            <div className="gv-card">
              ...
              <ul className="gv-course-list">
                {recentCourses.map((c) => {
                  ...
                  return (
                    <li key={c.courseId || c.title} className="gv-course-item">
                      <div className="gv-thumb-wrap">
                        <img src={c.thumbnailUrl || "/react-course.png"} alt={c.title || "Khóa học"} className="gv-course-thumb" />
                      ...
                      <div className="gv-course-info">
                        ...
                        <div className="gv-course-meta">
                          <span className={`gv-course-tag ${st.cls}`}>{st.label}</span>
                          <span className="gv-students">{new Intl.NumberFormat('vi-VN').format(c.studentCount || 0)} học viên</span>
                        </div>
                      </div>
            ...
            <div className="gv-card">
              ...
              <ul className="gv-activity-list">
                {recentActivities.map((a, idx) => {
                  const dot = dotClassForActivity(a.type, a.message)
                  return (
                    <li key={idx} className="gv-activity-item">
                      <span className={`gv-dot ${dot}`} />
                      <div className="gv-activity-text">
                        <div className="gv-activity-title">{a.message}</div>
                        {a.date && (<div className="gv-activity-sub">{new Date(a.date).toLocaleString('vi-VN')}</div>)}
                      </div>
                    </li>
                  )
                })}
